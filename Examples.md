...
- [Appendix: ‘Noisy’ Data Generation](#appendix-noisy-data-generation)
...
- Note, Consenrich is generally flexible with respect to available
  ...
  increased runtime due to swaps/disk I/O.
...
The aim of these experiments is to demonstrate simultaneously robust and
encompassing signal extraction from sets of noisy, heterogeneous
ATAC-seq data. A set of publicly available ATAC-seq data from human
lymphoblastoid cell lines is used to this end, as well as a set of
intentionally noisy samples generated by merging a GM12878 ChIP-Seq
control with subsamples of ATAC-seq data.
...
alt="Violin plots depicting the distribution of differences between Consenrich(Het10) and Consenrich(Het10 \cup Noisy&lt;5,10&gt;) signal track values (genome-wide)." />
...
alt="Chromosome-specific Pearson correlation coefficients between Consenrich(Het10) and Consenrich(Het10 \cup Noisy&lt;5,10&gt;) signal tracks." />
...
class="math inline">∪</span> Noisy&lt;5,10&gt;) signal
tracks.</figcaption>
</figure>
...
alt="Consenrich signal track outputs (middle panel, blue) given the ‘Het10’, ‘Het10 \cup Noisy5’, and ‘Het10 \cup Noisy10’ datasets as input. The alignment coverage tracks provided by IGV are displayed below for reference. The depicted 50kb region overlaps LYL1, which is highly expressed in lymphoblasts." />
...
which is highly expressed in lymphoblasts.</figcaption>
</figure>
...
paired controls. The dataset used is a set of ChIP-Seq experiments
targeting the TF *POL2RA* in colon tissue samples from independent
donors.

We likewise show utility of Consenrich signal tracks for
multi-sample/consensus peak calling using
[ROCCO](https://github.com/nolan-h-hamilton/ROCCO) (Hamilton and Furey
2023). A functional enrichment analysis on the returned peaks is
performed to evaluate relevant biological patterns. The BioConductor
package,
...
background-corrected treatment signals so that bias in across-sample
comparisons due to differences in sequencing depth is minimized.
...
promoters which is expected the target of this ChIP-Seq analysis
(POL2).*

# Mixed-Assay + Multi-Sample Analysis (DNase-seq $\cup$ ATAC-seq)

We aim to extract true signals in a multi-sample *and* multi-assay HTS
dataset (ATAC-seq and DNase-seq) of heart tissue samples. Adding further
heterogeneity to this dataset is the difference in read-lengths between
and paired-end/single-end data from the ATAC-seq and DNase-seq
...
discuss two common approaches below.
